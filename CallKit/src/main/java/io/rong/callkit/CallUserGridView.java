...
public class CallUserGridView extends ScrollView {
    ...
    public void addChild(String childId, UserInfo userInfo, String state) {
        ...
        AsyncImageView imageView = child.findViewById(R.id.rc_user_portrait);
        TextView name = child.findViewById(R.id.rc_user_name);
        name.setVisibility(enableTitle ? VISIBLE : GONE);
        TextView stateV = child.findViewById(R.id.rc_voip_member_state);
    ...
    public void removeChild(String childId) {
        ...
        for (LinearLayout resultContainer : containerList) {
            if (lastContainer == null) {
                LinearLayout child = resultContainer.findViewWithTag(childId);
                if (child != null) {
    ...
    public View findChildById(String childId) {
        ...
        for (int i = 0; i < containerCount; i++) {
            LinearLayout container = (LinearLayout) linearLayout.getChildAt(i);
            LinearLayout child = container.findViewWithTag(childId);
            if (child != null) {
                return child;
            }
        }
    ...
    public void updateChildInfo(String childId, UserInfo userInfo) {
        ...
        for (int i = 0; i < containerCount; i++) {
            ...
            if (child != null) {
                ...
                imageView.setAvatar(userInfo.getPortraitUri());
                if (enableTitle) {
                    TextView textView = child.findViewById(R.id.rc_user_name);
    ...
    public void updateChildState(String childId, String state) {
        ...
        for (int i = 0; i < containerCount; i++) {
            ...
            LinearLayout child = container.findViewWithTag(childId);
            if (child != null) {
                TextView textView = child.findViewById(R.id.rc_voip_member_state);
