...
import app.com.pgy.im.model.SealCSEvaluateItem;
        ...
        try {
            ...
            if (jsonArray != null && jsonArray.length() > 0) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    sealCSEvaluateItem.setQuestionFlag(jsonObject.optInt("isQuestionFlag", 0) == 1);
                    sealCSEvaluateItem.setScore(jsonObject.optInt("score"));
                    sealCSEvaluateItem.setScoreExplain(jsonObject.optString("scoreExplain"));
                    sealCSEvaluateItem.setTagMust(jsonObject.optInt("isTagMust", 0) == 1);
                    sealCSEvaluateItem.setInputMust(jsonObject.optInt("isInputMust", 0) == 1);
