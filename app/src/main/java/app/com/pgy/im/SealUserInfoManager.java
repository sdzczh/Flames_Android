...
import app.com.pgy.im.db.BlackList;
import app.com.pgy.im.db.BlackListDao;
import app.com.pgy.im.db.DBManager;
import app.com.pgy.im.db.Friend;
import app.com.pgy.im.db.FriendDao;
import app.com.pgy.im.db.GroupMember;
import app.com.pgy.im.db.GroupMemberDao;
import app.com.pgy.im.db.Groups;
import app.com.pgy.im.db.GroupsDao;
import app.com.pgy.im.db.UserInfoBean;
import app.com.pgy.im.server.network.async.AsyncTaskManager;
import app.com.pgy.im.server.network.async.OnDataListener;
import app.com.pgy.im.server.pinyin.CharacterParser;
import app.com.pgy.im.server.response.GetBlackListResponse;
import app.com.pgy.im.server.response.GetGroupInfoResponse;
import app.com.pgy.im.server.response.GetGroupMemberResponse;
import app.com.pgy.im.server.response.GetGroupResponse;
import app.com.pgy.im.server.response.GetTokenResponse;
...
import app.com.pgy.im.server.utils.RongGenerate;
...
public class SealUserInfoManager implements OnDataListener {
        ...
        if (mGroupsList != null && mGroupsList.size() > 0) {
            ...
            for (Groups group : mGroupsList) {
                ...
                /*if (groupMemberResponse != null && groupMemberResponse.getCode() == 200) {
                    fetchGroupCount++;
                    List<GetGroupMemberResponse.UserEntity> list = groupMemberResponse.getResult();
                    if (list != null && list.size() > 0) {
     ...
     */
    public List<Friend> addFriends(final List<GetUserInfosResponse.ResultEntity> list) {
        List<Friend> friendsList = new ArrayList<>();
        if (list != null && list.size() > 0) {
            syncDeleteFriends();
            for (GetUserInfosResponse.ResultEntity bean : list) {
                String headPath = bean.getHeadPath();
                String name = TextUtils.isEmpty(bean.getName())?"陌生人":bean.getName();
                String userId = TextUtils.isEmpty(bean.getPhone())?"1":bean.getPhone();
                String defaultAvatar = RongGenerate.generateDefaultAvatar(name,userId);
                headPath = TextUtils.isEmpty(headPath)?defaultAvatar:headPath;
                Friend friend = new Friend(userId,name,Uri.parse(headPath),name,
                            null, null, null, null,
                            CharacterParser.getInstance().getSpelling(name),
                            CharacterParser.getInstance().getSpelling(name));
                    if (TextUtils.isEmpty(headPath) || friend.getPortraitUri() == null || TextUtils.isEmpty(friend.getPortraitUri().toString())) {
                        String portrait = getPortrait(friend);
                        if (portrait != null) {
                            friend.setPortraitUri(Uri.parse(getPortrait(friend)));
                        }
                    }
                    LogUtils.w(TAG,"添加好友"+friend.getPortraitUri()+friend.toString());
     ...
     */
    public List<Groups> addGroups(final List<GetGroupResponse.GroupEntity> list) {
        if (list != null && list.size() > 0) {
            syncDeleteGroups();
            mGroupsList = new ArrayList<>();
            for (GetGroupResponse.GroupEntity group : list) {
                mGroupsList.add(new Groups(group.getGroupId(),group.getName(),group.getImgUrl(),""));
            }
     ...
     */
    public List<GroupMember> addGroupMembers(final List<GetGroupMemberResponse.UserEntity> list, final String groupID) {
        if (list != null && list.size() > 0) {
     ...
     * @return List<GroupMember> 群组成员列表
     */
    private List<GroupMember> getGroupMembers(String groupID) {
     ...
     */
    public void getGroupMembers(final String groupID,int memberNum, final ResultCallback<List<GetGroupMemberResponse.UserEntity>> callback) {
        if (TextUtils.isEmpty(groupID)) {
            if (callback != null) {
    ...
    private List<GroupMember> setCreatedToTop(List<GetGroupMemberResponse.UserEntity> groupMember, String groupID) {
        List<GroupMember> newList = new ArrayList<>();
        GroupMember created = null;
        for (GetGroupMemberResponse.UserEntity group : groupMember) {
            ...
            GroupMember newMember = new GroupMember(groupID,
                    group.getPhone(),
                    group.getName(),
                    Uri.parse(group.getHeadPath()),
                    group.getName(),
                    CharacterParser.getInstance().getSpelling(group.getName()),
                    CharacterParser.getInstance().getSpelling(group.getName()),
                    groupName,
