package huoli.com.pgy.Widgets.banner;
...
//    private BaseIndicator mIndicator;
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            ratio = ta.getFloat(R.styleable.BannerView_banner_ratio,0);
            if(mPageMargin > 0){
                mIsMargin = true;
            }
        } catch (Exception e) {
...
//            mIndicator.setCurrentPosition(smallPos);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (ratio == 0){
            ...
            int width = MathUtils.getWidthInPx(getContext());
            height = (int) ((width-2*mPageMargin+dp2px(32))/ratio);
        }
...
//        mIndicator.setCellCount(bannerData.size());
    ...
    private class InnerPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            Glide.with(getContext()).load(mBannerUrlList.get(position)).diskCacheStrategy(DiskCacheStrategy.ALL).into(bannerIv);
